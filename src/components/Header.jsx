...
export default function Header({ onBook }) {
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-black/95 border-b border-white/10 shadow-lg">
      ...
      {mobileOpen && (
        <div className="md:hidden px-4 pb-4 space-y-3 bg-black border-t border-white/10">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
